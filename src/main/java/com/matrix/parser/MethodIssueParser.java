...
public class MethodIssueParser {
	...
	private ParserResult getParserResult(String issueJson) {
		...
		try {
			...
			// 开始解析相关的堆栈
			HashMap<String, Object> content = Util.getGson().fromJson(issueJson,new TypeToken<HashMap<String, Object>>(){}.getType());
...
//			result.scene = getSceneStr(String.valueOf(content.get("detail")));
	...
	private String parserStack(String stack) {
		...
		for (String item : items) {
			...
			StringBuilder blank = new StringBuilder(temp[0]);
			for (int i = 0; i < s; i++) {
				blank.append(" ");
			}
			builder.append(blank).append(String.format(Locale.getDefault(), lineFormat, method, temp[2], temp[3]));
		}
